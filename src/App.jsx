...
const INIT_PRODUCTS = [
  { id: 1, name: "Robe de Soirée Élégante", category: "robes", gender: "femme", price: 18900, oldPrice: 24000, image: "https://images.unsplash.com/photo-1566479179817-c0a3a0d34e3a?w=400&q=80", images: [], badge: "-21%", brand: "LuxWear Collection", sizes: ["36","38","40","42"], pointures: [], colors: ["Noir","Rouge"], inStock: true },
  { id: 2, name: "Costume Homme Premium", category: "vetements", gender: "homme", price: 29900, oldPrice: null, image: "https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=400&q=80", images: [], badge: null, brand: "LuxWear Men", sizes: ["S","M","L","XL"], pointures: [], colors: ["Noir","Bleu marine"], inStock: true },
  { id: 3, name: "Sneakers Urban Luxe", category: "chaussures", gender: "homme", price: 12900, oldPrice: 17000, image: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&q=80", images: [], badge: "-24%", brand: "LuxStep", sizes: [], pointures: ["39","40","41","42","43"], colors: ["Blanc","Noir"], inStock: true },
  { id: 4, name: "Parfum Or Blanc", category: "parfums", gender: null, price: 9500, oldPrice: null, image: "https://images.unsplash.com/photo-1541643600914-78b084683702?w=400&q=80", images: [], badge: "NEW", brand: "LuxScent", sizes: [], pointures: [], colors: [], inStock: true },
  { id: 5, name: "Caftan Brodé Doré", category: "robes", gender: "femme", price: 24500, oldPrice: 31000, image: "https://images.unsplash.com/photo-1583391733981-8498e1b07c76?w=400&q=80", images: [], badge: "-21%", brand: "LuxWear Orient", sizes: ["38","40","42","44"], pointures: [], colors: ["Doré","Beige"], inStock: true },
  { id: 6, name: "Casquette Signature", category: "casquettes", gender: null, price: 5500, oldPrice: null, image: "https://images.unsplash.com/photo-1588850561407-ed78c282e89b?w=400&q=80", images: [], badge: null, brand: "LuxCap", sizes: ["S/M","L/XL"], pointures: [], colors: ["Noir","Blanc","Kaki"], inStock: true },
  { id: 7, name: "Sacoche Cuir Milano", category: "sacoches", gender: "femme", price: 17500, oldPrice: 22000, image: "https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=400&q=80", images: [], badge: "-20%", brand: "LuxBag", sizes: [], pointures: [], colors: ["Marron","Noir"], inStock: true },
  { id: 8, name: "Montre Prestige Or", category: "montres", gender: "homme", price: 52000, oldPrice: null, image: "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=400&q=80", images: [], badge: "EXCLU", brand: "LuxTime", sizes: [], pointures: [], colors: ["Doré","Argenté"], inStock: true },
];

// Catégories qui ont un filtre Homme/Femme
const CAT_GENDER = ["vetements","chaussures","sacoches","casquettes","montres","bijoux"];

// Tarifs livraison E-COMMERCE par wilaya
...
export default function LuxWear() {
  ...
  const addToCart = (p, e, size, color) => {
    ...
    setCart(c => {
      ...
      return ex ? c.map(x => x.cartKey === key ? { ...x, qty: x.qty + 1 } : x) : [...c, { ...p, qty: 1, cartKey: key, selectedSize: size || null, selectedColor: color || null }];
    });
  };
  ...
  const buildOrderText = o => {
    ...
    return `🛍️ NOUVELLE COMMANDE LUXWEAR\n\n👤 ${o.form.name}\n📞 ${o.form.phone}\n📍 ${o.form.wilaya}\n🏠 ${o.form.address}\n🚚 ${o.form.delivery === "domicile" ? "Domicile" : "Stop Desk"}\n⏱️ Délai: ${o.livraisonDelai || ""}\n\n${items}\n\n🛍️ Articles: ${o.totalArticles ? o.totalArticles.toLocaleString() : o.total.toLocaleString()} DA\n🚚 Livraison: ${o.livraisonPrix ? o.livraisonPrix.toLocaleString() : 0} DA\n💰 TOTAL: ${o.total.toLocaleString()} DA\n📅 ${o.date}`;
  };
  ...
  const sendEmail = async o => {
    try {
      await fetch("https://api.emailjs.com/api/v1.0/email/send", {
        ...
        body: JSON.stringify({ service_id: CONFIG.EMAILJS_SERVICE_ID, template_id: CONFIG.EMAILJS_TEMPLATE_ID, user_id: CONFIG.EMAILJS_PUBLIC_KEY, template_params: { to_email: CONFIG.OWNER_EMAIL, subject: `🛍️ Nouvelle commande — ${o.form.name}`, message: buildOrderText(o) } })
      });
    } catch (e) { console.log("EmailJS non configuré"); }
  };
  ...
  const handleOrder = async () => {
    ...
    const livraisonDelai = WILAYAS_TARIFS[form.wilaya]?.delai || "";
    const totalAvecLivraison = cartTotal + livraisonPrix;
    const newOrder = { id: Date.now(), items: [...cart], total: totalAvecLivraison, totalArticles: cartTotal, livraisonPrix, livraisonDelai, form: { ...form }, date: new Date().toLocaleDateString("fr-FR"), status: "En attente" };
    setOrders(o => [newOrder, ...o]);
    await sendEmail(newOrder);
    setCart([]); setCheckoutOpen(false); setSendingOrder(false); setOrderSuccess(true);
    setForm({ name: "", phone: "", wilaya: "", address: "", delivery: "domicile" }); setFormError("");
    setTimeout(() => setOrderSuccess(false), 4000);
  };

  const handleRegister = () => {
    if (!authForm.name || !authForm.email || !authForm.password) { setAuthError("Remplissez tous les champs obligatoires."); return; }
    if (accounts.find(a => a.email === authForm.email)) { setAuthError("Email déjà utilisé."); return; }
    const u = { ...authForm, id: Date.now() };
    setAccounts(a => [...a, u]); setUser(u); setAuthError("");
  };
  const handleLogin = () => {
    ...
    setUser(found); setAuthError("");
  };
  const handleLogout = () => { setUser(null); setAuthForm({ name: "", email: "", phone: "", password: "", avatar: "👩" }); setPage("home"); };
  ...
  const saveProduct = () => {
    ...
    const prod = { ...newProduct, price: Number(newProduct.price), oldPrice: newProduct.oldPrice ? Number(newProduct.oldPrice) : null };
    if (editingProduct && editingProduct.id) setProducts(ps => ps.map(p => p.id === editingProduct.id ? { ...prod, id: editingProduct.id } : p));
    else setProducts(ps => [...ps, { ...prod, id: Date.now() }]);
  ...
  const toggleStock = id => setProducts(ps => ps.map(p => p.id === id ? { ...p, inStock: !p.inStock } : p));
  const updateOrderStatus = (orderId, status) => setOrders(os => os.map(o => o.id === orderId ? { ...o, status } : o));
  const handleLogoTap = () => {
    setAdminTapCount(c => {
      const next = c + 1;
  ...
  const cancelledCount = orders.filter(o => o.status === "Annulé").length;
  ...
  return (
    <div style={{ fontFamily: "'Playfair Display',Georgia,serif", background: T.bg, minHeight: "100vh", color: T.text, maxWidth: 430, margin: "0 auto", position: "relative", overflowX: "hidden", transition: "background .3s,color .3s" }}>
      ...
      {/* TOAST */}
      {orderSuccess && (
        <div style={{ position: "fixed", top: 24, left: "50%", transform: "translateX(-50%)", background: G, color: "#0a0a0f", padding: "12px 24px", borderRadius: 30, fontWeight: 700, ...MF, zIndex: 999, boxShadow: "0 8px 32px rgba(212,175,55,0.5)", whiteSpace: "nowrap", textAlign: "center", animation: "pi2 .3s" }}>
          ✓ Commande envoyée !<br /><span style={{ fontSize: 11, fontWeight: 400 }}>Confirmation envoyée par email</span>
        </div>
      )}

      {/* HEADER */}
      <div style={{ padding: "20px 20px 12px", position: "sticky", top: 0, background: T.headerBg, zIndex: 50, borderBottom: `1px solid ${T.border}`, transition: "background .3s" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setSearchOpen(s => !s); if (searchOpen) setSearchQuery(""); }} style={{ background: searchOpen ? G : "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.25)", color: searchOpen ? "#0a0a0f" : "#d4af37", width: 38, height: 38, borderRadius: "50%", cursor: "pointer", fontSize: 16 }}>🔍</button>
        ...
        {searchOpen && (
          <div style={{ marginTop: 12, display: "flex", gap: 10, alignItems: "center", animation: "su .2s" }}>
            <input
              autoFocus
              className="if"
              ...
              style={{ flex: 1, borderRadius: 30, padding: "10px 18px", fontSize: 13 }}
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} style={{ background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.25)", color: "#d4af37", width: 34, height: 34, borderRadius: "50%", cursor: "pointer", fontSize: 14, flexShrink: 0 }}>✕</button>
            )}
          </div>
        )}
        {searchQuery && (
          <div style={{ marginTop: 8, ...MF, fontSize: 11, color: "rgba(212,175,55,0.7)" }}>
            {filtered.length} résultat{filtered.length !== 1 ? "s" : ""} pour "<span style={{ color: "#d4af37" }}>{searchQuery}</span>"
          </div>
        )}
      </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {["‹","›"].map((a, i) => <button key={i} onClick={() => scrollCat(i === 0 ? -1 : 1)} style={{ background: "rgba(212,175,55,0.1)", border: "1px solid rgba(212,175,55,0.2)", color: "#d4af37", width: 28, height: 28, borderRadius: "50%", cursor: "pointer", fontSize: 12 }}>{a}</button>)}
              </div>
            </div>
            ...
            {selectedCategory !== "all" && CAT_GENDER.includes(selectedCategory) && (
              <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                {[["homme","👨 Homme"],["femme","👩 Femme"]].map(([g, label]) => (
                  <button key={g} onClick={() => setSelectedGender(selectedGender === g ? null : g)} style={{ flex: 1, padding: "9px 0", borderRadius: 30, border: `1.5px solid ${selectedGender === g ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: selectedGender === g ? "rgba(212,175,55,0.12)" : "transparent", color: selectedGender === g ? "#d4af37" : T.textMuted, cursor: "pointer", ...MF, fontSize: 13, fontWeight: selectedGender === g ? 700 : 500, transition: "all .2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                    {label}
                    {selectedGender === g && <span style={{ fontSize: 10, background: "#d4af37", color: "#0a0a0f", padding: "1px 6px", borderRadius: 10, fontWeight: 700 }}>✓</span>}
                  </button>
                ))}
              </div>
            )}
            ...
            {filtered.map(p => (
              <div key={p.id} className="pc" onClick={() => { setSelectedProduct(p); setPhotoIndex(0); }}>
                ...
                <div style={{ padding: "12px 10px" }}>
                  ...
                  {p.inStock === false
                    ? <div style={{ width: "100%", padding: "8px 0", borderRadius: 30, background: T.surface2, color: T.textMuted, ...MF, fontSize: 11, fontWeight: 600, textAlign: "center" }}>🚫 Rupture</div>
          ...
          {!user ? (
            ...
            <div>
              <div style={{ background: "linear-gradient(135deg,#1a0a00 0%,#2d1810 40%,#0d1a3d 100%)", padding: "28px 24px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {user.phone && <div style={{ fontSize: 12, color: "rgba(212,175,55,0.7)", ...MF, marginTop: 3 }}>{user.phone}</div>}
                  </div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {[[orders.length, "Commandes"], [favorites.length, "Favoris"], [totalItems, "Articles"]].map(([v, l]) => (
              ...
              <div style={{ padding: "20px 16px" }}>
                {/* COMMANDES */}
                {profileTab === "commandes" && (orders.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "50px 20px", color: T.textFaint, ...MF }}>
                    <div style={{ fontSize: 44, marginBottom: 12 }}>📦</div>
                    <div style={{ fontSize: 14 }}>Aucune commande</div>
                    <button className="bg-gold" style={{ marginTop: 16, fontSize: 12, padding: "10px 24px" }} onClick={() => setPage("home")}>Commencer mes achats</button>
                  </div>
                ) : orders.map(o => {
                  ...
                  return (
                    <div key={o.id} className="oc">
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                        <div style={{ ...MF }}><div style={{ fontSize: 12, fontWeight: 700, color: "#d4af37" }}>#{String(o.id).slice(-6)}</div><div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>{o.date}</div></div>
                      ...
                      <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                        {o.items.map(i => <div key={i.cartKey} style={{ position: "relative" }}><img src={i.image} style={{ width: 50, height: 56, borderRadius: 10, objectFit: "cover" }} /><div style={{ position: "absolute", bottom: 2, right: 2, background: "#d4af37", color: "#0a0a0f", width: 16, height: 16, borderRadius: "50%", fontSize: 9, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{i.qty}</div></div>)}
                      </div>
                        ...
                        <div style={{ marginTop: 12 }}>
                          ...
                          {confirmCancelId === o.id ? (
                            <div style={{ background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.25)", borderRadius: 14, padding: 14, textAlign: "center" }}>
                              ...
                              <div style={{ fontSize: 11, color: T.textMuted, ...MF, marginBottom: 14 }}>Cette action est irréversible</div>
                              <div style={{ display: "flex", gap: 10 }}>
                                <button onClick={() => { setOrders(os => os.map(x => x.id === o.id ? { ...x, status: "Annulé" } : x)); setConfirmCancelId(null); }} style={{ flex: 1, padding: "10px 0", borderRadius: 20, border: "none", background: "rgba(255,80,80,0.85)", color: "#fff", cursor: "pointer", ...MF, fontSize: 12, fontWeight: 700 }}>✕ Oui, annuler</button>
                                <button onClick={() => setConfirmCancelId(null)} style={{ flex: 1, padding: "10px 0", borderRadius: 20, border: "1px solid rgba(212,175,55,0.3)", background: "transparent", color: "#d4af37", cursor: "pointer", ...MF, fontSize: 12, fontWeight: 600 }}>← Garder</button>
                              </div>
                            </div>
                          ) : (
                ...
                {profileTab === "favoris" && (favorites.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "50px 20px", color: T.textFaint, ...MF }}>
                    ...
                    <button className="bg-gold" style={{ marginTop: 16, fontSize: 12, padding: "10px 24px" }} onClick={() => setPage("home")}>Découvrir les produits</button>
                  </div>
                ) : (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    {favorites.map(p => (
                      <div key={p.id} className="fm" onClick={() => { setPage("home"); setTimeout(() => { setSelectedProduct(p); setPhotoIndex(0); }, 100); }}>
                        <div style={{ position: "relative" }}><img src={p.image} style={{ width: "100%", aspectRatio: "3/4", objectFit: "cover", display: "block" }} /><button className="hb" onClick={e => toggleFav(p, e)}>❤️</button></div>
                        <div style={{ padding: "10px 8px" }}>
                          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4 }}>{p.name}</div>
                          <div style={{ fontSize: 14, fontWeight: 700, color: "#d4af37", marginBottom: 8 }}>{Number(p.price).toLocaleString()} DA</div>
                          <button className="bg-gold" style={{ width: "100%", padding: "7px 0", fontSize: 11 }} onClick={e => { e.stopPropagation(); addToCart(p, e, null, null); }}>+ Panier</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* HISTORIQUE */}
                {profileTab === "historique" && (orders.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "50px 20px", color: T.textFaint, ...MF }}>
                    <div style={{ fontSize: 44, marginBottom: 12 }}>🕒</div>
                  ...
                  <div>
                    <div style={{ ...MF, fontSize: 12, color: T.textMuted, marginBottom: 16 }}>{orders.length} achat{orders.length > 1 ? "s" : ""}</div>
                    {orders.map(o => (
                      <div key={o.id} style={{ display: "flex", gap: 12, padding: "14px 0", borderBottom: `1px solid ${T.surface2}`, alignItems: "center" }}>
                        <div style={{ display: "flex" }}>{o.items.slice(0, 2).map(i => <img key={i.cartKey} src={i.image} style={{ width: 46, height: 52, borderRadius: 10, objectFit: "cover", marginRight: 4 }} />)}</div>
                        <div style={{ flex: 1, ...MF }}>
                          <div style={{ fontSize: 12, fontWeight: 600 }}>{o.items.length} article{o.items.length > 1 ? "s" : ""}</div>
                          <div style={{ fontSize: 11, color: T.textMuted }}>{o.date} · {o.form.wilaya}</div>
                        </div>
                        <div style={{ fontWeight: 700, color: "#d4af37" }}>{o.total.toLocaleString()} DA</div>
                      </div>
                    ))}
                  </div>
                ))}
                ...
                {/* EXPÉDIÉ */}
                {profileTab === "expedie" && (() => {
                  const shipped = orders.filter(o => o.status === "Expédié");
                  return shipped.length === 0 ? (
                    ...
                    <div key={o.id} className="oc" style={{ borderColor: "rgba(100,200,100,0.25)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                        <div style={{ ...MF }}><div style={{ fontSize: 12, fontWeight: 700, color: "#d4af37" }}>#{String(o.id).slice(-6)}</div><div style={{ fontSize: 11, color: T.textMuted }}>{o.date}</div></div>
                        <span style={{ background: "rgba(100,200,100,0.15)", color: "#6dcc6d", border: "1px solid rgba(100,200,100,0.3)", padding: "3px 10px", borderRadius: 20, fontSize: 10, ...MF, fontWeight: 700 }}>🚚 Expédié</span>
                      </div>
                      <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>{o.items.map(i => <img key={i.cartKey} src={i.image} style={{ width: 50, height: 56, borderRadius: 10, objectFit: "cover" }} />)}</div>
                      <div style={{ background: "rgba(100,200,100,0.07)", border: "1px solid rgba(100,200,100,0.15)", borderRadius: 10, padding: "10px 14px", ...MF, fontSize: 11, color: T.textMuted }}>📍 En cours de livraison → <span style={{ color: T.text, fontWeight: 600 }}>{o.form.wilaya}</span></div>
                    </div>
                  ));
                })()}
                ...
                {profileTab === "support" && (
                  <div>
                    <div style={{ textAlign: "center", marginBottom: 24 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 6 }}>Service Client LuxWear</div>
                      <div style={{ fontSize: 12, color: T.textMuted, ...MF }}>Disponible 7j/7 pour vous aider</div>
                    </div>
                    <div className="gl" />
                    {[
                      ...
                      { icon: "🎵", label: "TIKTOK", value: "@luxwear.dz", sub: "Nos vidéos & tendances", action: () => window.open(CONFIG.TIKTOK, "_blank"), bc: "rgba(255,255,255,0.15)", bg: "rgba(255,255,255,0.04)", color: "#fff" },
                      { icon: "👥", label: "FACEBOOK", value: "LuxWear DZ", sub: "Notre page officielle", action: () => window.open(CONFIG.FACEBOOK, "_blank"), bc: "rgba(24,119,242,0.25)", bg: "rgba(24,119,242,0.06)", color: "#1877f2" },
                    ].map(c => (
                      <div key={c.label} className="cc2" style={{ borderColor: c.bc, background: c.bg }} onClick={c.action}>
                        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                          <div style={{ width: 46, height: 46, borderRadius: "50%", background: c.bg, border: `1px solid ${c.bc}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{c.icon}</div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 10, color: c.color + "bb", ...MF, letterSpacing: 1, marginBottom: 4 }}>{c.label}</div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: c.color }}>{c.value}</div>
                            <div style={{ fontSize: 11, color: T.textMuted, ...MF, marginTop: 2 }}>{c.sub}</div>
                          </div>
                          <div style={{ fontSize: 18, color: c.color + "66" }}>›</div>
                        </div>
                      </div>
                    ))}
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: T.navBg, backdropFilter: "blur(16px)", borderTop: `1px solid ${T.border}`, display: "flex", zIndex: 50 }}>
        ...
        <div className="ni" onClick={() => setCartOpen(true)} style={{ position: "relative" }}>
          <span style={{ fontSize: 20 }}>🛒</span>Panier
          {cartCount > 0 && <span className="cc">{cartCount}</span>}
        </div>
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 200, overflowY: "auto", animation: "slideUp .3s" }}>
            <div style={{ position: "sticky", top: 0, background: T.bg, zIndex: 10, display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderBottom: `1px solid ${T.border}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 10, color: "rgba(212,175,55,0.6)", ...MF, letterSpacing: 2 }}>{selectedProduct.brand}</div>
              ...
              {allPhotos.length > 1 && (
                <>
                  ...
                  <div style={{ position: "absolute", bottom: 12, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 6 }}>
                    {allPhotos.map((_, i) => <div key={i} onClick={() => setPhotoIndex(i)} style={{ width: i === photoIndex ? 22 : 7, height: 7, borderRadius: 10, background: i === photoIndex ? "#d4af37" : "rgba(255,255,255,0.5)", cursor: "pointer", transition: "all .3s" }} />)}
                  </div>
            ...
            {allPhotos.length > 1 && (
              <div style={{ display: "flex", gap: 8, padding: "12px 16px", overflowX: "auto", background: T.surface, borderBottom: `1px solid ${T.border}` }}>
                {allPhotos.map((img, i) => <img key={i} src={img} onClick={() => setPhotoIndex(i)} style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 10, flexShrink: 0, cursor: "pointer", border: `2.5px solid ${i === photoIndex ? "#d4af37" : "transparent"}`, opacity: i === photoIndex ? 1 : 0.5, transition: "all .2s" }} />)}
              </div>
            )}
            ...
            <div style={{ padding: "20px 16px 120px" }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 12, lineHeight: 1.3 }}>{selectedProduct.name}</div>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ fontSize: 30, fontWeight: 700, color: "#d4af37" }}>{Number(selectedProduct.price).toLocaleString()} DA</span>
                  {selectedProduct.oldPrice && (
                    <div>
                      <span style={{ fontSize: 16, color: T.textFaint, textDecoration: "line-through", ...MF }}>{Number(selectedProduct.oldPrice).toLocaleString()} DA</span>
                      <div style={{ fontSize: 11, color: "#6dcc6d", ...MF, fontWeight: 600 }}>-{Math.round((1 - selectedProduct.price / selectedProduct.oldPrice) * 100)}% de réduction</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {selectedProduct.colors?.length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 12, display: "flex", justifyContent: "space-between" }}>
                    <span>COULEUR</span>
                    {selectedColor && <span style={{ color: "#d4af37", fontWeight: 700 }}>✓ {selectedColor}</span>}
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {selectedProduct.colors.map(cl => {
                      const cd = COULEURS.find(c => c.label === cl);
                      const active = selectedColor === cl;
                      return <button key={cl} onClick={() => setSelectedColor(active ? null : cl)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 20, border: `2px solid ${active ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: active ? "rgba(212,175,55,0.12)" : T.surface2, cursor: "pointer", transition: "all .2s" }}>
                        <div style={{ width: 20, height: 20, borderRadius: "50%", background: cd?.hex || "#888", border: "2px solid rgba(255,255,255,0.2)" }} />
                        <span style={{ ...MF, fontSize: 13, color: active ? "#d4af37" : T.textMuted, fontWeight: active ? 700 : 400 }}>{cl}</span>
                      </button>;
                    })}
                  </div>
                </div>
              ...
              {/* TAILLES */}
              {(() => {
                const hasTailles = selectedProduct.sizes?.length > 0;
                const hasPointures = selectedProduct.pointures?.length > 0;
                const options = hasPointures ? selectedProduct.pointures : hasTailles ? selectedProduct.sizes : [];
                const label = hasPointures ? "POINTURE" : hasTailles ? "TAILLE" : null;
                if (!label) return null;
                return (
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 11, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 12, display: "flex", justifyContent: "space-between" }}>
                      <span>{label}</span>
                      {selectedSize && <span style={{ color: "#d4af37", fontWeight: 700 }}>✓ {selectedSize}</span>}
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              ...
              <div style={{ background: T.surface, border: `1px solid ${T.borderCard}`, borderRadius: 16, padding: "14px 16px" }}>
                {[["🚚", "Livraison partout en Algérie", "Domicile ou Stop Desk"], ["✅", "Paiement à la livraison", "Vous payez à la réception"]].map(([icon, title, sub]) => (
                  <div key={title} style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 10 }}>
                    <span style={{ fontSize: 20 }}>{icon}</span>
                    <div style={{ ...MF }}><div style={{ fontSize: 12, fontWeight: 600 }}>{title}</div><div style={{ fontSize: 11, color: T.textMuted, marginTop: 1 }}>{sub}</div></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: T.navBg, backdropFilter: "blur(16px)", borderTop: `1px solid ${T.border}`, padding: "14px 16px", zIndex: 20 }}>
              {selectedProduct.inStock === false ? (
                <div style={{ padding: "14px 0", borderRadius: 30, background: T.surface2, color: T.textMuted, ...MF, fontSize: 15, fontWeight: 600, textAlign: "center" }}>🚫 Rupture de stock</div>
              ) : (
                <button className="bg-gold" style={{ width: "100%", fontSize: 15, padding: "14px 0" }} onClick={e => {
                  if ((selectedProduct.sizes?.length > 0 || selectedProduct.pointures?.length > 0) && !selectedSize) { alert("Veuillez choisir une taille / pointure"); return; }
                  if (selectedProduct.colors?.length > 0 && !selectedColor) { alert("Veuillez choisir une couleur"); return; }
                  addToCart(selectedProduct, e, selectedSize, selectedColor);
                  setSelectedSize(null); setSelectedColor(null); setSelectedProduct(null); setPhotoIndex(0);
        ...
        <div className="ov" onClick={() => setCartOpen(false)}>
          <div className="dr" onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 20, fontWeight: 700 }}>Mon Panier 🛒</div>
              <button onClick={() => setCartOpen(false)} style={{ background: "none", border: "none", color: "#d4af37", fontSize: 22, cursor: "pointer" }}>✕</button>
            </div>
            {cart.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 0", color: T.textFaint, ...MF }}>
                <div style={{ fontSize: 40, marginBottom: 10 }}>🛒</div><div>Votre panier est vide</div>
              </div>
            ) : (
              <>
                {cart.map(item => (
                  <div key={item.cartKey} style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "center", background: T.surface2, borderRadius: 14, padding: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {item.selectedSize && <div style={{ fontSize: 11, color: "#d4af37", ...MF }}>Taille : {item.selectedSize}</div>}
                      {item.selectedColor && <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 12, height: 12, borderRadius: "50%", background: COULEURS.find(c => c.label === item.selectedColor)?.hex || "#888" }} /><span style={{ fontSize: 11, color: "#d4af37", ...MF }}>{item.selectedColor}</span></div>}
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#d4af37", marginTop: 2 }}>{Number(item.price).toLocaleString()} DA</div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                      {["+", item.qty, "−"].map((v, i) => i === 1 ? <span key={i} style={{ ...MF, fontWeight: 700, fontSize: 14 }}>{v}</span> : <button key={i} onClick={() => updateQty(item.cartKey, i === 0 ? 1 : -1)} style={{ width: 26, height: 26, borderRadius: "50%", border: "1px solid rgba(212,175,55,0.3)", background: "transparent", color: "#d4af37", cursor: "pointer", fontSize: 14 }}>{v}</button>)}
                    </div>
                ...
                <div className="gl" />
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, ...MF }}>
                  <span style={{ fontSize: 14 }}>Total</span>
                  <span style={{ fontSize: 20, fontWeight: 700, color: "#d4af37" }}>{cartTotal.toLocaleString()} DA</span>
                </div>
      ...
      {/* CHECKOUT */}
      {checkoutOpen && (
        <div className="ov" onClick={() => setCheckoutOpen(false)}>
          <div className="dr" onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div style={{ fontSize: 20, fontWeight: 700 }}>Livraison 📦</div>
              <button onClick={() => setCheckoutOpen(false)} style={{ background: "none", border: "none", color: "#d4af37", fontSize: 22, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[["NOM COMPLET","text","Ex: Amina Benali","name"],["TÉLÉPHONE","text","0555 123 456","phone"]].map(([l,t,ph,k]) => (
                <div key={k}><div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>{l}</div><input className="if" type={t} placeholder={ph} value={form[k]} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))} /></div>
              ))}
              <div><div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>WILAYA</div>
                <select className="if" value={form.wilaya} onChange={e => setForm(f => ({ ...f, wilaya: e.target.value }))}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {[["domicile","🏠 Domicile"],["stopdesk","🏪 Stop Desk"]].map(([m,label]) => {
                    const tarif = WILAYAS_TARIFS[form.wilaya];
                    const prix = tarif ? (m === "domicile" ? tarif.domicile : tarif.stopdesk) : null;
                    return (
                      <button key={m} onClick={() => setForm(f => ({ ...f, delivery: m }))} style={{ flex: 1, padding: "10px 8px", borderRadius: 12, border: `1.5px solid ${form.delivery === m ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: form.delivery === m ? "rgba(212,175,55,0.1)" : "transparent", color: form.delivery === m ? "#d4af37" : T.textMuted, cursor: "pointer", ...MF, fontSize: 12, fontWeight: 600, textAlign: "center" }}>
                        {label}
                        {prix !== null && prix !== undefined && <div style={{ fontSize: 11, fontWeight: 700, color: form.delivery === m ? "#d4af37" : T.textFaint, marginTop: 3 }}>{prix === 0 ? "Non disponible" : `${prix.toLocaleString()} DA`}</div>}
                      </button>
                    );
                  })}
                </div>
                {/* DÉLAI + INFO */}
                {form.wilaya && WILAYAS_TARIFS[form.wilaya] && (
                  <div style={{ marginTop: 10, background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.18)", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontSize: 18 }}>⏱️</span>
                    <div style={{ ...MF, fontSize: 12 }}>
                      <span style={{ color: T.textMuted }}>Délai de livraison : </span>
                      <span style={{ color: "#d4af37", fontWeight: 700 }}>{WILAYAS_TARIFS[form.wilaya].delai}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* RÉCAP PRIX */}
            {form.wilaya && WILAYAS_TARIFS[form.wilaya] && (
              <div style={{ background: T.surface2, borderRadius: 14, padding: "12px 16px", marginBottom: 12, ...MF }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textMuted, marginBottom: 8 }}>
                  <span>Livraison ({form.delivery === "domicile" ? "Domicile" : "Stop Desk"})</span>
                  <span style={{ color: "#d4af37" }}>
                    {(() => { const p = form.delivery === "domicile" ? WILAYAS_TARIFS[form.wilaya].domicile : WILAYAS_TARIFS[form.wilaya].stopdesk; return p === 0 ? "Non disponible" : `${p.toLocaleString()} DA`; })()}
                  </span>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700 }}>
                  ...
                  <span style={{ color: "#d4af37" }}>
                    {(() => { const p = form.delivery === "domicile" ? WILAYAS_TARIFS[form.wilaya].domicile : WILAYAS_TARIFS[form.wilaya].stopdesk; return (cartTotal + (p || 0)).toLocaleString(); })()} DA
                  </span>
                </div>
              </div>
            )}
            {!form.wilaya && (
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12, ...MF }}>
                <span>Total articles</span>
                <span style={{ fontSize: 18, fontWeight: 700, color: "#d4af37" }}>{cartTotal.toLocaleString()} DA</span>
              </div>
            )}
            <button className="bg-gold" style={{ width: "100%", fontSize: 14 }} disabled={sendingOrder} onClick={handleOrder}>
              {sendingOrder ? "⏳ Envoi en cours..." : "✓ Confirmer la commande"}
            </button>
          </div>
        </div>
      )}
      ...
      {/* ADMIN */}
      {adminOpen && (
        <div className="ov" onClick={() => { setAdminOpen(false); setAdminAuth(false); setAdminPassword(""); }}>
          <div className="dr" style={{ maxHeight: "95vh" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            ...
            {!adminAuth ? (
              <div>
                <div style={{ textAlign: "center", marginBottom: 24 }}>
                  <div style={{ fontSize: 50, marginBottom: 10 }}>🔒</div>
                  <div style={{ fontSize: 14, color: T.textMuted, ...MF }}>Entrez le mot de passe administrateur</div>
                </div>
                <input className="if" type="password" placeholder="Mot de passe admin..." value={adminPassword} onChange={e => setAdminPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAdminLogin()} style={{ marginBottom: 12 }} />
              ...
              <div>
                <div style={{ display: "flex", borderBottom: `1px solid ${T.border}`, marginBottom: 20 }}>
                  {[["produits","🛍️ Produits"],["commandes","📦 Commandes"]].map(([id, label]) => (
                    <button key={id} className={`tb ${adminTab === id ? "ac" : ""}`} onClick={() => { setAdminTab(id); setAdminSearchQuery(""); }} style={{ fontSize: 13 }}>{label}</button>
                  ))}
                </div>
                ...
                {adminTab === "produits" && (
                  editingProduct === null ? (
                    <div>
                      <button className="bg-gold" style={{ width: "100%", marginBottom: 12, fontSize: 13 }} onClick={() => { setEditingProduct({}); setNewProduct(EMPTY_PRODUCT); }}>＋ Ajouter un nouveau produit</button>
                      ...
                      <div style={{ position: "relative", marginBottom: 14 }}>
                        <input
                          ...
                          style={{ paddingRight: adminSearchQuery ? 36 : 16 }}
                        />
                        {adminSearchQuery && (
                          <button onClick={() => setAdminSearchQuery("")} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#d4af37", cursor: "pointer", fontSize: 16 }}>✕</button>
                        )}
                      </div>

                      <div style={{ ...MF, fontSize: 12, color: T.textMuted, marginBottom: 12 }}>
                        {adminSearchQuery
                          ? `${products.filter(p => p.name.toLowerCase().includes(adminSearchQuery.toLowerCase()) || p.brand.toLowerCase().includes(adminSearchQuery.toLowerCase())).length} résultat(s) pour "${adminSearchQuery}"`
                        ...
                        <div key={p.id} className="ac2" style={{ borderColor: p.inStock === false ? "rgba(255,80,80,0.3)" : "rgba(212,175,55,0.15)" }}>
                          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ fontSize: 10, color: T.textMuted, ...MF }}>{CATEGORIES.find(c => c.id === p.category)?.label}{p.gender ? ` · ${p.gender === "homme" ? "👨 Homme" : "👩 Femme"}` : " · ✦ Mixte"}</div>
                            </div>
                    ...
                    <div>
                      ...
                      {(newProduct.images?.length > 0 || newProduct.image) && (
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                          {[newProduct.image, ...(newProduct.images || [])].filter(Boolean).map((img, i) => (
                            <div key={i} style={{ position: "relative" }}>
                              <img src={img} style={{ width: 70, height: 80, objectFit: "cover", borderRadius: 10, border: i === 0 ? "2px solid #d4af37" : "1px solid rgba(212,175,55,0.2)" }} onError={e => e.target.style.display = "none"} />
                              {i === 0 && <div style={{ position: "absolute", bottom: 2, left: 2, background: "#d4af37", color: "#0a0a0f", fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 6, ...MF }}>MAIN</div>}
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {[["NOM DU PRODUIT *","text","Ex: Robe Élégante","name"],["MARQUE / BRAND","text","Ex: LuxWear","brand"]].map(([label,type,ph,key]) => (
                          <div key={key}><div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>{label}</div><input className="if" type={type} placeholder={ph} value={newProduct[key] || ""} onChange={e => setNewProduct(p => ({ ...p, [key]: e.target.value }))} /></div>
                        ))}

                        <div style={{ display: "flex", gap: 10 }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>PRIX ACTUEL (DA) *</div>
                            <input className="if" type="number" placeholder="Ex: 12000" value={newProduct.price || ""} onChange={e => {
                              const price = e.target.value;
                              const oldP = newProduct.oldPrice;
                              const autoBadge = oldP && price && Number(oldP) > Number(price) ? `-${Math.round((1 - Number(price) / Number(oldP)) * 100)}%` : newProduct.badge;
                              setNewProduct(p => ({ ...p, price, badge: (oldP && price && Number(oldP) > Number(price)) ? `-${Math.round((1 - Number(price) / Number(oldP)) * 100)}%` : p.badge }));
                            }} />
                          </div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>ANCIEN PRIX (DA)</div>
                            <input className="if" type="number" placeholder="Ex: 15000" value={newProduct.oldPrice || ""} onChange={e => {
                              const oldP = e.target.value;
                              const price = newProduct.price;
                              setNewProduct(p => ({ ...p, oldPrice: oldP, badge: (oldP && price && Number(oldP) > Number(price)) ? `-${Math.round((1 - Number(price) / Number(oldP)) * 100)}%` : p.badge }));
                            }} />
                          </div>
                        </div>

                        <div>
                          <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>
                            BADGE {newProduct.price && newProduct.oldPrice && Number(newProduct.oldPrice) > Number(newProduct.price) && <span style={{ color: "#6dcc6d", fontWeight: 700 }}>✓ -{Math.round((1 - Number(newProduct.price) / Number(newProduct.oldPrice)) * 100)}% auto</span>}
                          </div>
                          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            <input className="if" placeholder="Ex: NEW ou EXCLU" value={newProduct.badge || ""} onChange={e => setNewProduct(p => ({ ...p, badge: e.target.value }))} style={{ flex: 1 }} />
                            {newProduct.badge && <div style={{ background: G, color: "#0a0a0f", padding: "6px 12px", borderRadius: 20, ...MF, fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{newProduct.badge}</div>}
                          </div>
                        </div>

                        <div>
                          <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 6 }}>PHOTO PRINCIPALE *</div>
                          <input className="if" placeholder="https://... lien photo principale" value={newProduct.image || ""} onChange={e => setNewProduct(p => ({ ...p, image: e.target.value }))} />
                        </div>
                        ...
                        <div>
                          ...
                          {(newProduct.images || []).map((img, i) => (
                            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                              <input className="if" placeholder={`https://... photo ${i + 2}`} value={img} onChange={e => setNewProduct(p => ({ ...p, images: p.images.map((x, j) => j === i ? e.target.value : x) }))} style={{ flex: 1 }} />
                              <button onClick={() => setNewProduct(p => ({ ...p, images: p.images.filter((_, j) => j !== i) }))} style={{ background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.3)", color: "#ff6060", width: 32, height: 32, borderRadius: "50%", cursor: "pointer", fontSize: 14, flexShrink: 0 }}>✕</button>
                            </div>
                          ))}
                        ...
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 10 }}>
                            {[[null,"✦ Mixte"],["homme","👨 Homme"],["femme","👩 Femme"]].map(([g, label]) => (
                              <button key={String(g)} onClick={() => setNewProduct(p => ({ ...p, gender: g }))} style={{ flex: 1, padding: "8px 0", borderRadius: 20, border: `1.5px solid ${newProduct.gender === g ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: newProduct.gender === g ? "rgba(212,175,55,0.12)" : "transparent", color: newProduct.gender === g ? "#d4af37" : T.textMuted, cursor: "pointer", ...MF, fontSize: 12, fontWeight: newProduct.gender === g ? 700 : 500 }}>{label}</button>
                            ))}
                          </div>
                        </div>
                        ...
                        {(CAT_TAILLES.includes(newProduct.category) || newProduct.category === "casquettes") && (
                          <div>
                            <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 8 }}>TAILLES DISPONIBLES <span style={{ color: T.textFaint, fontWeight: 400 }}>({(newProduct.sizes || []).length} sélectionnées)</span></div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                              {(newProduct.category === "casquettes" ? TAILLES_CASQUETTES : newProduct.category === "robes" ? TAILLES_ROBES : TAILLES_VETEMENTS).map(s => {
                                const active = (newProduct.sizes || []).includes(s);
                                return <button key={s} onClick={() => setNewProduct(p => ({ ...p, sizes: active ? p.sizes.filter(x => x !== s) : [...(p.sizes || []), s] }))} style={{ minWidth: 44, padding: "7px 10px", borderRadius: 10, border: `1.5px solid ${active ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: active ? "rgba(212,175,55,0.15)" : "transparent", color: active ? "#d4af37" : T.textMuted, cursor: "pointer", ...MF, fontSize: 13, fontWeight: active ? 700 : 400 }}>{s}</button>;
                              })}
                            </div>
                          </div>
                        )}

                        {CAT_POINTURES.includes(newProduct.category) && (
                          <div>
                            <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 8 }}>POINTURES DISPONIBLES <span style={{ color: T.textFaint, fontWeight: 400 }}>({(newProduct.pointures || []).length} sélectionnées)</span></div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                              {POINTURES.map(s => {
                                const active = (newProduct.pointures || []).includes(s);
                                return <button key={s} onClick={() => setNewProduct(p => ({ ...p, pointures: active ? p.pointures.filter(x => x !== s) : [...(p.pointures || []), s] }))} style={{ minWidth: 44, padding: "7px 10px", borderRadius: 10, border: `1.5px solid ${active ? "#d4af37" : "rgba(212,175,55,0.2)"}`, background: active ? "rgba(212,175,55,0.15)" : "transparent", color: active ? "#d4af37" : T.textMuted, cursor: "pointer", ...MF, fontSize: 13, fontWeight: active ? 700 : 400 }}>{s}</button>;
                              })}
                            </div>
                          </div>
                        )}
                        ...
                        <div>
                          <div style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", ...MF, letterSpacing: 1, marginBottom: 8 }}>COULEURS DISPONIBLES <span style={{ color: T.textFaint, fontWeight: 400 }}>({(newProduct.colors || []).length} sélectionnées)</span></div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                            {COULEURS.map(c => {
                              const active = (newProduct.colors || []).includes(c.label);
                              return <button key={c.label} onClick={() => setNewProduct(p => ({ ...p, colors: active ? p.colors.filter(x => x !== c.label) : [...(p.colors || []), c.label] }))} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 20, border: `1.5px solid ${active ? "#d4af37" : "rgba(255,255,255,0.1)"}`, background: active ? "rgba(212,175,55,0.1)" : "transparent", cursor: "pointer" }}>
                                <div style={{ width: 18, height: 18, borderRadius: "50%", background: c.hex, border: "1.5px solid rgba(255,255,255,0.2)" }} />
                                <span style={{ ...MF, fontSize: 11, color: active ? "#d4af37" : T.textMuted, fontWeight: active ? 700 : 400 }}>{c.label}</span>
                              </button>;
                            })}
                          </div>
                        </div>
                ...
                {adminTab === "commandes" && (
                  <div>
                    {cancelledCount > 0 && (
                      <div style={{ background: "rgba(255,68,68,0.1)", border: "1.5px solid rgba(255,68,68,0.35)", borderRadius: 14, padding: "12px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ fontSize: 22 }}>🚨</div>
                        <div style={{ ...MF }}><div style={{ fontSize: 13, fontWeight: 700, color: "#ff6060" }}>{cancelledCount} commande{cancelledCount > 1 ? "s" : ""} annulée{cancelledCount > 1 ? "s" : ""} !</div><div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>Un client a annulé sa commande</div></div>
                      </div>
                    )}
                    <div style={{ ...MF, fontSize: 12, color: T.textMuted, marginBottom: 16 }}>{orders.length} commande{orders.length !== 1 ? "s" : ""} au total</div>
                    {orders.length === 0 ? (
                      <div style={{ textAlign: "center", padding: "40px 0", color: T.textFaint, ...MF }}><div style={{ fontSize: 40, marginBottom: 10 }}>📭</div><div>Aucune commande</div></div>
                    ) : [...orders].sort((a, b) => { const r = s => s === "Annulé" ? 0 : s === "En attente" ? 1 : 2; return r(a.status) - r(b.status); }).map(o => {
                      const ss = statusStyle(o.status);
                      return (
                        <div key={o.id} className="ac2" style={o.status === "Annulé" ? { borderColor: "rgba(255,68,68,0.4)", background: "rgba(255,68,68,0.04)" } : {}}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                            <div style={{ ...MF }}><div style={{ fontSize: 13, fontWeight: 700, color: "#d4af37" }}>#{String(o.id).slice(-6)}</div><div style={{ fontSize: 11, color: T.textMuted }}>{o.date}</div></div>
                          ...
                          <div style={{ background: T.surface2, borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
                            <div style={{ ...MF, fontSize: 12 }}>
                              ...
                              <div style={{ color: T.textMuted }}>🚚 {o.form.delivery === "domicile" ? "Domicile" : "Stop Desk"}</div>
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                            {o.items.map(i => (
                              <div key={i.cartKey} style={{ position: "relative", textAlign: "center" }}>
                                <img src={i.image} style={{ width: 46, height: 52, borderRadius: 8, objectFit: "cover", display: "block" }} />
                                <div style={{ position: "absolute", bottom: 2, right: 2, background: "#d4af37", color: "#0a0a0f", width: 15, height: 15, borderRadius: "50%", fontSize: 9, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{i.qty}</div>
                                {i.selectedSize && <div style={{ fontSize: 9, color: "#d4af37", ...MF, fontWeight: 700 }}>{i.selectedSize}</div>}
                              </div>
                            ))}
                          </div>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                          ...
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {["En attente","Expédié","Livré","Annulé"].map(s => {
                              const ss2 = statusStyle(s);
                              return <button key={s} onClick={() => updateOrderStatus(o.id, s)} style={{ padding: "5px 10px", borderRadius: 20, border: `1px solid ${o.status === s ? ss2.color : "rgba(255,255,255,0.1)"}`, background: o.status === s ? ss2.bg : "transparent", color: o.status === s ? ss2.color : T.textMuted, cursor: "pointer", ...MF, fontSize: 10, fontWeight: 600 }}>{s === "En attente" ? "⏳" : s === "Expédié" ? "🚚" : s === "Livré" ? "✅" : "❌"} {s}</button>;
                            })}
                          </div>
                          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                            <button onClick={() => window.open(`https://wa.me/${o.form.phone.replace(/\s/g, "")}?text=${encodeURIComponent(`Bonjour ${o.form.name}, votre commande LuxWear #${String(o.id).slice(-6)} est confirmée ✓`)}`, "_blank")} style={{ flex: 1, padding: "8px 0", borderRadius: 20, border: "1px solid rgba(37,211,102,0.3)", background: "rgba(37,211,102,0.08)", color: "#25d366", cursor: "pointer", ...MF, fontSize: 11, fontWeight: 600 }}>💬 WhatsApp client</button>
                            <button onClick={() => setOrders(os => os.filter(x => x.id !== o.id))} style={{ padding: "8px 14px", borderRadius: 20, border: "1px solid rgba(255,80,80,0.3)", background: "rgba(255,80,80,0.08)", color: "#ff6060", cursor: "pointer", ...MF, fontSize: 11, fontWeight: 600, flexShrink: 0 }}>🗑 Supprimer</button>
                          </div>
                        </div>
                      );
                    })}
